<div class="space-y-6" style="zoom: 90%">
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
            ...
            <div class="flex items-center justify-between">
                <h3 class="text-lg font-semibold text-gray-800 flex items-center">
                    <svg class="w-5 h-5 mr-2 text-rose-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 3v4M15 3v4M3 9h18M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M9 13h6M9 17h6" />
                    </svg>
                ...
                <div class="flex items-center gap-4">
                    <div class="bg-white rounded-lg px-4 py-2 shadow-sm border border-gray-200">
                        <span class="text-sm text-gray-600">Total ventanillas:</span>
                        <span class="text-lg font-bold text-rose-600 ml-2">{{ $ventanillas->total() }}</span>
                    </div>
                    
                    <button wire:click="openCreateModal"
                        class="inline-flex items-center px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg transition-all duration-150 transform hover:scale-105 shadow-sm">
        ...
        <div class="p-6">
                ...
                <div class="lg:col-span-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search"
                            class="block w-full pl-10 pr-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all"
                            placeholder="Número, descripción...">
                    </div>
                </div>

                {{-- Tipo Ubicación Filter --}}
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">
                        Tipo Ubicación
                    </label>
                    <select wire:model.live="filterTipoUbicacion"
                        class="block w-full px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 transition-all">
                        <option value="">Todos los tipos</option>
                        @foreach($tiposUbicacionTicket as $tipo)
            ...
            @if ($search || $filterEstado !== '' || $filterTipoUbicacion !== '')
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse($ventanillas as $ventanilla)
                        <tr class="hover:bg-gray-50 transition-colors duration-150">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    <div class="w-10 h-10 rounded-lg bg-gradient-to-br from-rose-500 to-pink-500 flex items-center justify-center text-white font-bold text-lg shadow-sm mr-3">
                                        {{ $ventanilla->numero }}
                                    </div>
                                    <span class="text-sm font-semibold text-gray-900">Ventanilla {{ $ventanilla->numero }}</span>
                                </div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600">{{ $ventanilla->descripcion ?? 'Sin descripción' }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    ...
                                    {{ $ventanilla->tipoUbicacionTicket->ubicacion ?? 'N/A' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                    {{ $ventanilla->estado == 1 ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800' }}">
                                    {{ $ventanilla->estado == 1 ? 'Activo' : 'Inactivo' }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="text-sm text-gray-900">
                                    {{ \Carbon\Carbon::parse($ventanilla->fecha_creacion)->format('d/m/Y') }}
                                </div>
                                <div class="text-xs text-gray-500">
                                    {{ \Carbon\Carbon::parse($ventanilla->fecha_creacion)->format('H:i') }}
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <div class="flex items-center justify-center space-x-2">
                                    {{-- Edit Button --}}
                                    <button wire:click="edit({{ $ventanilla->id }})"
                                    ...
                                    {{-- Delete Button --}}
                                    <button wire:click="confirmDelete({{ $ventanilla->id }})"
                        ...
                        <tr>
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-gray-500 text-lg font-medium">No se encontraron ventanillas</p>
        ...
        <div class="bg-gray-50 px-6 py-4 border-t border-gray-200">
            <div class="flex flex-col sm:flex-row items-center justify-between gap-4">
                <div class="text-sm text-gray-700">
                    Mostrando
                    <span class="font-semibold text-gray-900">{{ $ventanillas->firstItem() ?? 0 }}</span>
                    a
                    <span class="font-semibold text-gray-900">{{ $ventanillas->lastItem() ?? 0 }}</span>
                    de
                    <span class="font-semibold text-gray-900">{{ $ventanillas->total() }}</span>
                    ventanillas
                </div>
                <div>
                    {{ $ventanillas->links() }}
                ...
                <div
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input type="number" wire:model="numero"
                                    class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500"
                                    placeholder="Ej: 1" min="1" max="999">
                                @error('numero')
                            ...
                            {{-- Tipo Ubicación Ticket --}}
                            <div>
                                <label class="block text-sm font-semibold text-gray-700 mb-2">
                                    Tipo de Ubicación *
                                </label>
                                <select wire:model="id_tipo_ubicacion_ticket"
                                    class="block w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500">
                                ...
                                @error('id_tipo_ubicacion_ticket')
                                    <span class="text-red-500 text-xs mt-1">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                        ...
                        <button type="button" wire:click="store"
                            class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-rose-600 hover:bg-rose-700 transition-all">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7" />
                            </svg>
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog"
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0">
                ...
                <div
                    class="relative inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full z-50">
                    {{-- Header --}}
                    <div class="bg-gradient-to-r from-violet-600 to-rose-600 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <h3 class="text-xl font-bold text-white flex items-center">
                                <svg class="w-6 h-6 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                        ...
                        <button type="button" wire:click="update"
                            class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-violet-600 hover:bg-violet-700 transition-all">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M5 13l4 4L19 7" />
                            </svg>
            ...
            <div class="flex items-center justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:p-0">
                ...
                <div
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left flex-1">
                                ...
                                <div class="mt-3">
                                    <p class="text-sm text-gray-600">
                                        ¿Estás seguro de que deseas eliminar esta ventanilla?
                                    </p>
                    ...
                    <div class="bg-gray-50 px-6 py-4 flex flex-col-reverse sm:flex-row sm:justify-end gap-3">
                        ...
                        <button type="button" wire:click="deleteVentanilla"
